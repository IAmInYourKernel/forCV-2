...
class _AddAmbulanceScreenState extends State<AddAmbulanceScreen> {
  ...
  Future getImage() async {
    ...
    if (image != null) {
      ...
      setState(() {
        if (croppedFile != null) {
          ambulancePic = File(croppedFile.path);
        }
      });
    }
  }
                ...
                Column(children: [
                  ...
                  Padding(
                    ...
                    child: OvalButton(
                        ...
                        onPressed: () async {
                          if (carID.text.length < 2) {
                            displayToastMessage(
                                "Name must be atleast of 2 digits", context);
                          } else {
                            newDriver.ambulance = Ambulance();
                            newDriver.driver!.carID = carID.text;
                            newDriver.ambulance!.carID = carID.text;
                            newDriver.ambulance!.picture = ambulancePic;
                            newDriver.ambulance!.carManufacturer =
                                manufacturer.text;
                            newDriver.ambulance!.carModel = model.text;
                            newDriver.ambulance!.type =
                                AmbulanceType(id: int.parse(typeID.text));
                            newDriver.ambulance!.year = int.parse(year.text);

                            await ServiceAssistant.createNewDriver()
                                .then((value) {
